...
export async function GET(req: NextRequest) {
  ...
  const [empRes, attRes, advRes] = await Promise.all([
    ...
    supabase.from('advances').select('*').eq('month', month),
  ]);
  ...
  const data = (empRes.data || []).map(emp => {
    ...
    const gross_salary = Math.round(total_days * Number(emp.daily_rate) * 100) / 100;
    ...
    return {
      ...
      daily_rate: Number(emp.daily_rate), gross_salary,
      total_advances, net_salary: Math.round((gross_salary - total_advances) * 100) / 100,
