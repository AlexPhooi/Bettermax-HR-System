    ...
    <div className="p-4 md:p-6 max-w-7xl mx-auto">
      ...
      <Modal open={showAccModal} onClose={() => setShowAccModal(false)}
        ...
        <form onSubmit={handleAccSubmit} className="space-y-4">
          ...
          {/* Admin force-reset note */}
          {editAccForm.newPassword && (
            <div className="rounded-lg border border-blue-200 bg-blue-50 px-3 py-2.5">
              <p className="text-xs text-blue-700">
                🔑 <strong>Force reset</strong> — the new password will take effect immediately. Share it with the staff member so they can log in.
              </p>
            </div>
          )}
      ...
      <div className="mt-10">
        ...
        >
          <span className="flex items-center gap-2 font-semibold text-sm">
            🗑️ Bin
            {binData && binData.staff.length > 0 && (
              <span className="bg-red-100 text-red-600 text-xs font-bold px-2 py-0.5 rounded-full">
                {binData.staff.length}
              </span>
            )}
          </span>
        ...
        {binOpen && (
          <div className="mt-3 card p-0 overflow-hidden">
            <div className="px-5 py-3 border-b border-bg bg-gray-50">
              <p className="text-xs text-gray-500">Deleted staff accounts are stored here. Restore to recover.</p>
            </div>
