  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 style={{ fontFamily: 'Georgia, serif', fontSize: 22, fontWeight: 700, color: '#2C1A0E' }}>
            Operations Dashboard
          </h1>
          <p style={{ color: '#C49A6C', fontSize: 13 }}>Active projects overview</p>
        </div>
        {isManager && (
          <Link href="/operations/projects" className="btn btn-primary btn-sm">
            + New Project
          </Link>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
        {[
          ...
          <div key={s.label} className="card" style={{ marginBottom: 0, padding: '16px 20px' }}>
            ...
            <div style={{ fontFamily: 'Georgia, serif', fontSize: 26, fontWeight: 700, color: '#2C1A0E', lineHeight: 1 }}>
              {s.value}
            </div>
            <div style={{ fontSize: 11, color: '#C49A6C', marginTop: 4, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
              {s.label}
            </div>
          </div>
        ))}
      ...
      {isManager && data.total_contract_value > 0 && (
        <div className="alert alert-info mb-6" style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <span style={{ fontSize: 18 }}>💰</span>
          <span>Total contract value across active projects: <strong>{fmt(data.total_contract_value)}</strong></span>
        </div>
      )}
          ...
          {data.projects.map(p => {
            ...
            return (
              <Link key={p.id} href={`/operations/projects/${p.id}`}
                ...
                {p.location && (
                  <p style={{ fontSize: 12, color: '#6B4A00', marginBottom: 8 }}>📍 {p.location}</p>
                )}

                {/* Progress bar */}
                <div style={{ marginBottom: 12 }}>
                  <div className="flex justify-between mb-1">
                    <span style={{ fontSize: 11, color: '#C49A6C' }}>Progress</span>
                  ...
                  <div style={{ height: 6, background: '#F5EDD6', borderRadius: 3, overflow: 'hidden' }}>
                    <div style={{
                      ...
                      width: `${progress}%`,
                      background: progress >= 80 ? '#27500A' : progress >= 50 ? '#C9A84C' : '#C49A6C',
                    }} />
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, fontSize: 11 }}>
                  {p.foreman_name && (
                    <span style={{ color: '#6B4A00' }}>👷 {p.foreman_name}</span>
                  )}
                  ...
                  {p.no_log_alert ? (
                    <span style={{ color: '#791f1f', fontWeight: 600 }}>
                      ⚠️ {p.latest_log_date ? `No log ${p.days_since_log}d` : 'No logs yet'}
                    </span>
                  ) : (
                    <span style={{ color: '#27500A' }}>
                      ✅ Log {p.days_since_log === 0 ? 'today' : `${p.days_since_log}d ago`}
                    </span>
                  )}
                </div>
              </Link>
