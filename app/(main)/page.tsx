...
const QUICK = [
  ...
  { href: '/attendance', icon: '📋', label: 'Record Attendance' },
  { href: '/salary',     icon: '💰', label: 'Calculate Salary' },
...
export default function DashboardPage() {
  ...
  const stats = data ? [
    { label: 'Total Active Employees', value: data.active_employees, color: 'border-primary' },
    ...
    { label: 'Permits Expiring (60 days)', value: data.permits_expiring, color: 'border-warn', valColor: data.permits_expiring > 0 ? 'text-warn' : '' },
  ] : [];
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
          {QUICK.map(q => (
            <Link key={q.href} href={q.href}
              className="block bg-white border-2 border-gray-200 hover:border-primary rounded-lg p-4 text-center transition-all hover:shadow-md hover:-translate-y-0.5 group">
