...
// ── Brand icon — real Better Max Builder logo (public/logo-icon.png) ──
function BrandIcon({ size = 36 }: { size?: number }) {
  return (
    <Image src="/logo-icon.png" alt="" width={313} height={214} priority
...
export default function Navbar() {
  ...
  return (
    <>
      {/* ── Top navigation bar ──────────────────────────────────── */}
      <nav style={{ background: '#2E1810', borderBottom: '2px solid #C9962E' }}
          ...
          <button onClick={() => setOpen(o => !o)}
            ...
            <div className="leading-tight">
              <p style={{ fontFamily: 'Georgia, serif', fontSize: 13, fontWeight: 700, letterSpacing: 1, margin: 0 }}>
                <span style={{ color: '#E8D5A3' }}>BETTER</span> <span style={{ color: '#C9962E' }}>MAX</span>
              </p>
              <p style={{ fontFamily: 'Arial, sans-serif', color: '#C9962E', fontSize: 8.5, fontWeight: 700, letterSpacing: 2.5, margin: 0 }}>
                BUILDER
              </p>
            </div>
          </button>
              ...
              <li key={l.href}>
                <Link href={l.href}
                  style={isActive(l.href)
                    ? { background: '#4A2E1F', color: '#C9962E', borderBottom: '2px solid #C9962E', fontWeight: 700 }
                    : { color: '#C49A6C' }}
                  className="px-3 py-1.5 rounded-t text-sm transition-colors block relative hover:text-[#E8D5A3] hover:bg-[#4A2E1F]"
                  ...
                  {isAdmin && l.href === '/attendance' && pendingCount > 0 && (
                    <span style={{ background: '#C9962E', color: '#2E1810' }}
          ...
          <div className="ml-auto flex items-center gap-2">
            {loaded && username && (
              <span className="hidden md:block text-xs px-2.5 py-1 rounded"
                style={{ color: '#C49A6C', background: '#4A2E1F' }}>
                {username}
              </span>
            )}
              ...
              <div className="relative" ref={bellRef}>
                ...
                {bellOpen && (
                  <div className="absolute right-0 top-full mt-2 w-80 bg-white rounded-lg shadow-xl z-50 overflow-hidden"
                    style={{ border: '1px solid #C9962E' }}>
                    <div className="px-4 py-3" style={{ background: '#2E1810', borderBottom: '1px solid #C9962E' }}>
                    ...
                    <div className="px-4 py-2.5" style={{ borderTop: '1px solid #E8D5A3', background: '#FAF5E9' }}>
                      <Link href="/staff" onClick={() => setBellOpen(false)}
                        className="text-xs font-semibold hover:underline" style={{ color: '#C9962E' }}>
                        View all employees →
                      </Link>
                    </div>
                  </div>
            ...
            <button onClick={logout}
              ...
              onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = '#4A2E1F'; (e.currentTarget as HTMLElement).style.color = '#E8D5A3'; }}
      ...
      {open && (
        <div className="md:hidden fixed top-14 left-0 right-0 z-40 shadow-lg"
          style={{ background: '#2E1810', borderBottom: '2px solid #C9962E' }}>
          {loaded && modules.length > 1 && (
            <div className="flex gap-2 px-6 py-3" style={{ borderBottom: '1px solid #4A2E1F' }}>
              {modules.map(m => (
          ...
          {navLinks.map(l => (
            <Link key={l.href} href={l.href} onClick={() => setOpen(false)}
              ...
              style={isActive(l.href)
                ? { background: '#4A2E1F', color: '#C9962E', fontWeight: 700, borderLeft: '3px solid #C9962E' }
          ...
          {loaded && username && (
            <div className="px-6 py-2.5 text-xs" style={{ color: '#6B4A00', borderTop: '1px solid #4A2E1F' }}>
